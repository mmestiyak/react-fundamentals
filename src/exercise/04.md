...
In order to reuse part of code, we use function exactly like that in our React Code, in order to reuse shareable jsx, we use functions, in React these are known as React Component, it can be created with React.createElement and the first string is instead of built in components like ('div, img') we will be using a function with PascalNotation and give props and children just as we use jsx and from that function we pass into React.createElement, we will get a object and can access all of the thing we pass through the React.createElement Function and use as per our needs to make that Function aka React Component.
