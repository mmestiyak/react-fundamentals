...
ReactJS uses the DOM Web API's but it gives us a very declaravie way to interact with DOM compared to how imperatively, in a very low way we need to work with DOM web API in order to interact with DOM, as a whole, it makes it easy to interact with DOM with lots of abstraction and very efficiency in updating DOM as they keep all changes in virtual DOM, use diff algorithm to make minimum changes required in the actual DOM
## Background
