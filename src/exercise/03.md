...
Instead of writing Code with raw react API's, it would be nice to able to write like html and that feature brought by JSX, it's a html like syntax-sugar on top of raw React API's. JSX couldn't be understand by browser because it's not javaScript, so we need to use a compiler and here it's babel, as it's written in javaScript so we can run and compile on the fly but it's not efficient to use on browser in real scenarios rather compile code before deployment.
## Background
