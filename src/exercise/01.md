...
In order to show the view that is written with the document html, the browser converts the html to Document Object Model (DOM). Because Browser can render view with DOM not with html, DOM is simply just a object  model/version of the html document file. Browser exposes the DOM to javascript and so it's possible with javascript to read, write and delete everything from the DOM which is the programming interface of the view, so it's possible to change everything on the view with javascript by chaning DOM, that's how with the help of javascript we can even generate the view without writing any html code and that's how modern javascript frameworks like React, Vue are working.
